//----------------------------------------------------------------------------//
/*
This do-file rebuilds the Belo-Lin-Yang model-implied ICS using the
linear formula

    ICS = (0.0459 * dEI_std + (-0.0783) * dDK_std) * 100   [percent]
    (coefficients read directly from the cell formula in
     Data\model implied ICS 20220419.xlsx, not the rounded 0.046 / 0.078
     stated in the posted CSV header)

where dEI is the change in the share of equity-issuing firms and dDK is the
change in aggregate (DLTT+DLC)/PPENT (top 1% by ME excluded).

Sample is the CRSP/Compustat-merged universe under new CIZ filters,
non-utility / non-financial, on NYSE/AMEX/Nasdaq.

Standardization: mean / sd computed over 1964-2026 (full window).
The author's original 1964-2020 model-implied series is preserved verbatim;
new years (post 2020) are appended on the same units.

Author: Henry Chen
Date: 2026-04-28
*/
//----------------------------------------------------------------------------//


//----------------------------------------------------------------------------//
**# Setup
//----------------------------------------------------------------------------//
clear
set more off

global wrds      "wrds"
global crspstart "1962-12-01"
global crspend   "2026-12-31"
global compstart "1963-01-01"
global compend   "2026-12-31"

global data   "`c(pwd)'\Data"
global output "`c(pwd)'\Output"
global temp   "`c(pwd)'\Temp"

foreach dirpath in temp output {
    mata: st_numscalar("OK", direxists("$`dirpath'"))
    if scalar(OK) == 0 mkdir "$`dirpath'"
}


//----------------------------------------------------------------------------//
**# Step 1: CRSP monthly with CIZ common-stock + exchange filters via CCM
//----------------------------------------------------------------------------//
clear
odbc load, exec("SELECT l.gvkey, l.permno, l.linkprim, l.linkdt, l.linkenddt, s.permco, s.mthcaldt, s.mthprc, s.shrout, s.siccd, s.primaryexch FROM (SELECT gvkey, lpermno AS permno, linkprim, linkdt, linkenddt FROM crsp_a_ccm.ccmxpf_linktable WHERE linktype IN ('LU','LC') AND linkprim IN ('P','C')) AS l JOIN (SELECT permco, permno, mthcaldt, mthprc, shrout, siccd, primaryexch FROM crsp.msf_v2 WHERE securitytype='EQTY' AND securitysubtype='COM' AND sharetype='NS' AND issuertype IN ('ACOR','CORP') AND usincflg='Y' AND primaryexch IN ('N','A','Q') AND conditionaltype='RW' AND tradingstatusflg='A') AS s ON s.permno=l.permno WHERE s.mthcaldt >= '$crspstart' AND s.mthcaldt <= '$crspend'") dsn("$wrds")

replace linkenddt = date("$crspend","YMD") if missing(linkenddt)
keep if inrange(mthcaldt, linkdt, linkenddt)
drop if inrange(siccd, 4900, 4999) | inrange(siccd, 6000, 6999)

save "$temp\crsp_monthly", replace


//----------------------------------------------------------------------------//
**# Step 2: Compustat annual
//----------------------------------------------------------------------------//
clear
odbc load, exec("SELECT gvkey, fyear, datadate, sstk, prstkc, dv, dltt, dlc, ppent FROM comp_na_daily_all.funda WHERE datadate BETWEEN DATE '$compstart' AND DATE '$compend' AND indfmt='INDL' AND popsrc='D' AND datafmt='STD' AND consol='C' AND curcd='USD' AND costat IN ('A','I')") dsn("$wrds")

drop if missing(fyear)
sort gvkey fyear datadate
bys gvkey fyear: keep if _n == _N
save "$temp\compustat_annual", replace


//----------------------------------------------------------------------------//
**# Step 4: December ME panel
//   ME at firm level = sum across permnos within (gvkey, December year)
//   siccd / primaryexch carried from primary permno (linkprim='P')
//----------------------------------------------------------------------------//
use "$temp\crsp_monthly", clear
keep if month(mthcaldt) == 12
gen calyear = year(mthcaldt)
gen me = abs(mthprc) * shrout / 1000   // millions

gen byte is_primary = (linkprim == "P")
gsort gvkey calyear -is_primary
by gvkey calyear: gen byte pick = (_n == 1)

preserve
    keep if pick == 1
    keep gvkey calyear siccd primaryexch
    tempfile primary_attrs
    save `primary_attrs'
restore

collapse (sum) me, by(gvkey calyear)
merge 1:1 gvkey calyear using `primary_attrs', keep(match) nogen

save "$temp\me_dec", replace


//----------------------------------------------------------------------------//
**# Step 5: Firm-year master
//----------------------------------------------------------------------------//
use "$temp\compustat_annual", clear
gen year = fyear
keep if inrange(year, 1963, 2026)

// Merge ME at year t
preserve
    use "$temp\me_dec", clear
    rename calyear year
    rename me me_dec_t
    keep gvkey year me_dec_t siccd primaryexch
    tempfile me_t
    save `me_t'
restore
merge 1:1 gvkey year using `me_t', keep(match) nogen

// Merge ME at year t-1
preserve
    use "$temp\me_dec", clear
    gen year = calyear + 1
    rename me me_dec_lag
    keep gvkey year me_dec_lag
    tempfile me_lag
    save `me_lag'
restore
merge 1:1 gvkey year using `me_lag', keep(match master) nogen

// Re-apply utility/finance exclusion via CRSP siccd at year t
drop if inrange(siccd, 4900, 4999) | inrange(siccd, 6000, 6999)

gen me_avg = 0.5 * (me_dec_t + me_dec_lag)

save "$temp\firm_year", replace


//----------------------------------------------------------------------------//
**# Step 6: dEI
//----------------------------------------------------------------------------//
use "$temp\firm_year", clear

gen byte has_threshold = !missing(sstk) & !missing(me_avg) & me_avg > 0
gen byte equity_issuer = (sstk >= 0.03 * me_avg) if has_threshold

preserve
    keep if has_threshold
    collapse (mean) ei = equity_issuer (sum) n_issuers = equity_issuer ///
             (count) n_firms = equity_issuer, by(year)
    replace ei = ei * 100   // percent
    sort year
    tsset year
    gen dei = ei - L.ei     // percentage-point change
    save "$temp\dei", replace
restore


//----------------------------------------------------------------------------//
**# Step 7: dDK
//   Drop top 1% by me_dec_t each year. Missing dltt, dlc treated as 0.
//   ppent must be positive.
//----------------------------------------------------------------------------//
use "$temp\firm_year", clear

bysort year: egen me_p99 = pctile(me_dec_t), p(99)
drop if me_dec_t > me_p99 & !missing(me_dec_t)

replace dltt = 0 if missing(dltt)
replace dlc  = 0 if missing(dlc)
drop if missing(ppent) | ppent <= 0

collapse (sum) sum_dltt=dltt sum_dlc=dlc sum_ppent=ppent ///
         (count) n_firms_dk = ppent, by(year)
gen dk = (sum_dltt + sum_dlc) / sum_ppent * 100   // percent
sort year
tsset year
gen ddk = dk - L.dk                                // percentage-point change

save "$temp\ddk", replace


//----------------------------------------------------------------------------//
**# Step 8: Combine + standardize over 1964-2026
//----------------------------------------------------------------------------//
use "$temp\dei", clear
merge 1:1 year using "$temp\ddk", nogen

keep if inrange(year, 1964, 2026)

foreach v in dei ddk {
    qui summ `v'
    gen `v'_std = (`v' - r(mean)) / r(sd)
}

save "$temp\factors", replace


//----------------------------------------------------------------------------//
**# Step 9: Raw model-implied ICS
//----------------------------------------------------------------------------//
gen ICS = (0.0459 * dei_std + (-0.0783) * ddk_std) * 100   // percent
keep year ei dei dei_std dk ddk ddk_std ICS n_firms n_firms_dk
order year ei dei dei_std dk ddk ddk_std ICS n_firms n_firms_dk

save "$temp\ics_raw", replace


//----------------------------------------------------------------------------//
**# Step 12: Append to author's original; write extended xlsx + dta + csv
//   Author's Sheet1 has 8 columns: year, frac, levEx1, dfrac, dlev,
//   zfrac, zlev, ICS (all components — frac and levEx1 in percent).
//   We preserve those rows verbatim through last_old, then append our
//   computed values for years > last_old in the same column structure.
//----------------------------------------------------------------------------//
// Read author's full Sheet1. Header row uses merged cells over D-E and F-G,
// so import without firstrow and rename by column letter.
import excel using "$data\model implied ICS 20220419.xlsx", ///
    sheet("Sheet1") cellrange(A2) clear
rename A year
rename B frac
rename C levEx1
rename D dfrac
rename E dlev
rename F zfrac
rename G zlev
rename H ICS

qui summ year
local last_old  = r(max)
local first_old = r(min)
di as text "Author's series spans `first_old' - `last_old'"

tempfile author_full
save `author_full'

// Our values for years > last_old, renamed to author's column convention
use "$temp\ics_raw", clear
keep if year > `last_old'
keep year ei dei dei_std dk ddk ddk_std ICS
rename ei      frac
rename dei     dfrac
rename dei_std zfrac
rename dk      levEx1
rename ddk     dlev
rename ddk_std zlev
order year frac levEx1 dfrac dlev zfrac zlev ICS
tempfile our_extension
save `our_extension'

// Combine: author's verbatim + our extension
use `author_full', clear
append using `our_extension'
sort year

save "$output\model_implied_ICS_20260428", replace

// Sheet1 — extended series, same 8-column layout as author's file
export excel year frac levEx1 dfrac dlev zfrac zlev ICS ///
    using "$output\model implied ICS 20260428.xlsx", ///
    sheet("Sheet1") firstrow(variables) replace

// Components_ours — full computed pipeline 1964-2026 with diagnostics
// (separate sheet since author's file has no equivalent)
use "$temp\ics_raw", clear
export excel year ei dei dei_std dk ddk ddk_std ICS n_firms n_firms_dk ///
    using "$output\model implied ICS 20260428.xlsx", ///
    sheet("Components_ours") firstrow(variables) sheetmodify

// CSV mirror of Sheet1
use "$output\model_implied_ICS_20260428", clear
export delimited using "$output\model_implied_ICS_20260428.csv", replace

di as result "Done. Extended model-implied ICS written to Output\."
